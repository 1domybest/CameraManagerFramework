...
extension CameraManager {
    ...
    public  func findDeviceForMultiSession(withPosition position: AVCaptureDevice.Position) -> AVCaptureDevice? {
        ...
        if position == .back {
            if supportedDeviceSets.isEmpty {
                if let defaultFrontCamera = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .back) {
                      // 멀티 세션과 호환되는 포맷을 선택합니다.
                    self.setCameraFormat(camera: defaultFrontCamera)
                    backCameraMinimumZoonFactor = defaultFrontCamera.minAvailableVideoZoomFactor
                    backCameraMaximumZoonFactor = defaultFrontCamera.maxAvailableVideoZoomFactor
                    backCameraDefaultZoomFactor = 1.0
                    backCameraCurrentZoomFactor = backCameraDefaultZoomFactor
